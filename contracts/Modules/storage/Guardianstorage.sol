...
contract GuardianStorage is IGuardianStorage {
 ...
 /**
    ...
    modifier onlyModule(address _wallet) {
        ...
        require(TwoUserMultisig(payable(_wallet)).isModuleEnabled(msg.sender), "GS: methold only callable by an enabled module");
        _;
    }
    ...
    function addGuardian(address _wallet, address _guardian) external onlyModule(_wallet) {
        ...
        require(!TwoUserMultisig(payable(_wallet)).isOwner(_guardian), "GS: guardian cannot be an owner");
